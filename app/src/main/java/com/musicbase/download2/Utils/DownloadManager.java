package com.musicbase.download2.Utils;
...
import com.musicbase.download2.observer.DateObserverable;
import com.musicbase.download2.thread.DownloadThread1;
...
public class DownloadManager {
	 ...
	 * @param doc
	 */
	public void startForActivity(Context context,DocInfo doc) {
		// 判断数据库里面是否包含下载记录
		...
		if (null != info) {
			if (info.getStatus() == DataBaseFiledParams.DONE) {
				...
				ActivityUtils.deleteBookFormSD(context,info.getBookId(), info.getName());
				start(doc);
	 ...
	 * @param doc
	 */
	public void cancel(Context context,DocInfo doc) {
		DownloadThread1 thread = getThreadByDocInfo(doc);
		if (thread != null) {
			stopDownload(doc);
		}
		mDataBaseHelper.deleteValue(doc);
		ActivityUtils.deleteBookFormSD(context,doc.getBookId(), doc.getName());
		ActivityUtils.deleteFoder(new File(ActivityUtils.getSDPath(context,doc.getBookId()), doc.getBookName()));
		if (doc.getBookName().endsWith(".zip")) {
			String[] s = doc.getBookName().split("\\.");
			if (!TextUtils.isEmpty(s[0]))
				ActivityUtils.deleteFoder(new File(ActivityUtils.getSDPath(context,doc.getBookId()), s[0]));
		}
		ActivityUtils.deleteFoder(new File(ActivityUtils.getSDPath(context,doc.getBookId()), "__MACOSX"));
...
//		}
	 ...
	 * @param info
	 */
	public void downloadFailed(Context context,DocInfo info) {
		if (listeners != null) {
			for (DownloadListener listener : listeners) {
	 ...
	 * 下载监听
	 */
	private static List<DownloadListener> listeners = new ArrayList<DownloadListener>();
	 ...
	 * 判断正在下载
	 */
	public boolean isDownloading(DetailBean.DataBean.ResourceListBean item) {
		...
		if (docInfos != null && docInfos.size() > 0) {
			for (DocInfo docInfo : docInfos) {
				if (docInfo != null && !TextUtils.isEmpty(docInfo.getName().toString())) {
					if (!TextUtils.isEmpty(item.getResourceSaveName()) && item.getResourceSaveName().equals(docInfo.getName().toString())) {
						return true;
					}
				}
			}
